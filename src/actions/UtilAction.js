...
export const getActionAPI = ({
  ...
  return async (dispatch, getState) => {
    ...
    try {
      ...
      if (sessionKey) {
        const session = getValueFromObjectByKeys(data, ['data', 'session']);
        console.log('session', sessionKey, SessionQueue.isLatestSession(sessionKey, session));
        if (!SessionQueue.isLatestSession(sessionKey, session)) { throw {}; }
      }
      console.log('GetActionAPI', data);
      return data;
    } catch (err) {
      console.log('UtilActionErr', err);
      const error = getValueFromObjectByKeys(err, ['errors']);
      let errMsg = '';
      if (error) {
        if (isString(error)) {
          errMsg = error;
